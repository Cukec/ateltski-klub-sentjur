...
class UploadController
{
    ...
    public function __construct(Config $config, AuthInterface $auth, Filesystem $storage, TmpfsInterface $tmpfs)
    {
        ...
        $this->conn = new mysqli('localhost', 'root', '', 'ak-sentjur');
    ...
    public function upload(Request $request, Response $response)
    {
        ...
        if ($chunks_size >= $total_size) {
            ...
            if ($res) {
                ...
                $url = 'repository/galerija/' . $destination . '/' . $file_name;
                $stmt = $this->conn->prepare("INSERT INTO test_image (url, upload_datetime) VALUES (?, NOW())");
                $stmt->bind_param("s", $url);
                $stmt->execute();

                return $response->json('Stored');
            } else {
                return $response->json('Error storing file');
            }
        }
